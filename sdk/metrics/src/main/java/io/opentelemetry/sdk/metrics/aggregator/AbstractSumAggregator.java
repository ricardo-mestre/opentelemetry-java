...
abstract class AbstractSumAggregator<T> extends AbstractAggregator<T> {
  ...
  /**
   * Resolve whether the aggregator is stateful. For the special case {@link
  ...
  /**
   * Resolve the merge strategy. The merge strategy is SUM in all cases except where temporality is
   * {@link AggregationTemporality#DELTA} and instrument type is {@link InstrumentType#SUM_OBSERVER}
   * or {@link InstrumentType#UP_DOWN_SUM_OBSERVER}. In these special cases, the observed values are
   * cumulative sums so we must take a diff to compute the delta sum.
   *
   * @param instrumentType the instrument type
   * @param temporality the temporality
